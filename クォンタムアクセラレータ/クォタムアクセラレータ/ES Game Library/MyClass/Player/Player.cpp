...
void Player::Update()
{
	if(Input.GetPadInputDown(0,5) && UiDataBase.GetItemSize() >= 4){ PlayerAccelaretor();}
...
void Player::SetAccelaretorParameter()
{
	...
	accelaretor_parameter._start_velocity = 0.8f;
}

void Player::PlayerAccelaretor()
{
...
void Player::AttackHit(int attack_type)
{
	...
	case ATTACKTYPE::ITEM:
		p_player_processor.ChangeState(new Player::GETITEM(&p_player_processor));
		break;
...
Player::RECOVERY::RECOVERY(CPlayerStateProcessor* owner) : _owner(owner)
{
	...
	_owner->p_player->observer.IsAddTiemr(_owner->p_player->AddTimeNum);
}
